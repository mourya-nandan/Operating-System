...
freewalk(pagetable_t pagetable)
{
  ...
  for(int i = 0; i < 512; i++){
    ...
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
      ...
      panic("freewalk: leaf");
      /*printf("freewalk: unexpected leaf PTE\n");
      printf("  pagetable=%p i=%d pte=0x%lx\n",
            pagetable, i, (unsigned long)pte);
      printf("  PTE bits: V=%d R=%d W=%d X=%d U=%d A=%d\n",
            !!(pte & PTE_V), !!(pte & PTE_R), !!(pte & PTE_W),
            !!(pte & PTE_X), !!(pte & PTE_U), !!(pte & PTE_A));
      panic("freewalk: leaf (debug above)");
      */
    }
  }
